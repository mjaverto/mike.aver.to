...
This is the battle-tested version of that recipe. It is not theoretical. Almost every section below has a "this cost me an hour" story attached, and I've kept those in deliberately, because the gotchas *are* the value. The happy path is short. The detours are where you learn how the thing actually works.

The runtime is **Troublemaker** ([github.com/tinyfatco/troublemaker](https://github.com/tinyfatco/troublemaker)), an open-source Slack/Telegram agent shell. Troublemaker is built on **Pi** (`@earendil-works/pi-agent-core`, whose binary is `crawdad`), and it's modeled on Mario Zechner's "mom" ambient-agent pattern — an agent that *watches* a channel and stays silent (`yield_no_action`) unless it's actually addressed. Everything here generalizes to any similar runtime; substitute names as needed.
...
1. **A Docker container on your own host.** Inside it runs the Troublemaker/Pi runtime as a single long-lived process (`crawdad`). It exposes an internal HTTP port (`3002`) with `/health` and `/status` endpoints, and it bind-mounts a `/data` working directory from the host so the agent's workspace, persona files, and skills survive container rebuilds. The container runs as **root** with `--sandbox=host` — meaning the agent's shell/tools run directly in the container as root. (That's a real security posture decision; we'll get to it.)
...
You: "@Kit who are you?" in #dev
   ...
   → Troublemaker reacts with :ack: (instant "received" signal)
...
- SSH access to that host. If your user isn't root but *is* in the `docker` group, that's enough — Docker runs as root, so you can create root-owned host directories via a throwaway container even without passwordless `sudo` (shown below).
...
- Comfort with reading a TypeScript source tree. You *will* end up grepping the runtime's source to answer questions the docs don't. Budget for it.
...
We pinned Troublemaker to a specific commit SHA (call it `<SHA>`). The instinct is right; the execution has a trap. The upstream Dockerfile **clones `--depth 1 HEAD`** — i.e. it bakes whatever is on `main` at *build time*, not your pinned SHA. So a `local/troublemaker-kit:<SHA>` tag built from that Dockerfile is a lie: the tag says one thing, the contents are "latest main."
...
```dockerfile
# instead of:  RUN git clone --depth 1 https://github.com/tinyfatco/troublemaker .
# do this, against a tree you've already checked out at <SHA>:
COPY . /opt/troublemaker
...
The cause: Troublemaker's startup sequence (`initMetadata` in its shared `slack-base` adapter) does more than `auth.test`. Before it will connect, it runs, in order:
...
Troublemaker is a thin shell on Pi. Pi builds the agent through a `ModelRegistry` and supports several providers. We used Pi's **`openai-codex`** provider, which authenticates with a **ChatGPT OAuth token** (Plus/Pro subscription) — *not* the standalone Codex CLI, and *not* a raw API key.
...
(If you'd rather skip OAuth entirely, set `MOM_MODEL_PROVIDER=openai` and supply `OPENAI_API_KEY` — but that changes which model you're billing and how.)
...
You might be tempted to just `gh auth login` inside the container via the web/device flow. **Don't** — that web flow grants gh's broad default scopes (`repo`, `read:org`, `gist`, **`workflow`**), i.e. a long-lived token that can reach every repo the account can touch and rewrite Actions workflows. Given we run `--sandbox=host` (agent runs bash as root, reads untrusted Slack/PR content, prompt-injectable), a broad long-lived token is exactly the risk to avoid.
...
services:
  kit-troublemaker:
    image: local/troublemaker-kit:<SHA>
    container_name: kit-troublemaker
...
…and the references to `/worktrees/…`, `/repos/*`, `auth.json`, and `kit.env` were *gone*. The agent ran for a while with **partially-gutted rules** and nobody noticed, because the file looked fine at a glance. **Always re-read the persona file after writing it and confirm the backticked tokens survived.** This is exactly the kind of silent corruption that turns "never push to main" into "never push to" — a rule that no longer says anything.

---

## 6) Behavior tuning (threading, the ack reaction, the double-post bug, hiding "Thinking", concise)

This is where Troublemaker's "ambient mom" heritage shows, and where you'll do real source patches. We kept every edit surgical and saved as a re-applyable `.diff`, because building from local source means you're maintaining a small fork — patches won't carry across a SHA bump automatically.
...
- **Reference helper scripts by full path** (`bash /data/skills/<name>/run.sh`). A symlink into `/usr/local/bin` won't survive a rebuild — that path isn't a mounted volume.
...
Troublemaker ships a "fat-skills" set (browser tools, PDF generation, etc.) in the image, but they are **not loaded into the prompt** unless you add a `--skills` dir to the command:

```yaml
command: ["--adapter=slack:socket", "--sandbox=host", "--port=3002", "--skills=/opt/fat-skills", "/data"]
```

`--skills` is repeatable and **adds dirs at lowest priority** — your workspace `/data/skills` still loads and **wins name collisions**, so your own skills are never shadowed by the bundled set. On boot you'll see something like `Extra skills dirs: /opt/fat-skills`.
...
- **Exec into the container** to inspect creds and the workspace. It runs as root, so:
  ```bash
  ssh your-host
  docker compose -f /srv/kit/config/docker-compose.yml exec kit-troublemaker bash
...
1. **The Dockerfile floats `HEAD`.** A SHA tag built from the upstream Dockerfile is a lie. Build from a checked-out tree (`COPY . /opt/troublemaker`).
